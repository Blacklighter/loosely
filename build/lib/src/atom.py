...
from base import LooseBase, FileType
...
#     from process import Wrapper, Filter
#     classes = utils.get_all_classes_in_module(__name__, lambda c: issubclass(c, LooseAtom) and c != LooseAtom) + [Wrapper, Filter]
#     for c in classes:
#         if c.is_matched_from_path(path):
#             return c
#     else:
#         raise Exception(f"No implementation of LooseAtom class for extension '{utils.get_ext(path)}'")
