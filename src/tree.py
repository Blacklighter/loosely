...
from loosely.atom import LooseAtomArray, LooseAtomDict, LooseAtom
...
class LooseTree(LooseBase):
    ...
    def get_children_paths(self):
        paths = []
        if self.can_load:
            for obj, trace, parent in deep_iter_items(self.get_data(), lambda x: isinstance(x, LooseBase), iter_get_data=False):
                paths.append(obj.load_path)
        return paths
    ...
    def _clean_excessive_children(self):
        if self.load_path is not None:
            ...
            # print(list(old_children_paths))
            # print(self.load_path, old_children_paths)
            children_paths = self.get_children_paths()
            excessive_paths = list(set(old_children_paths).difference(children_paths))
...
class LooseJSONTree(LooseTree):
    ...
    def _load_data(cls, path):
        ...
        for v, trace, parent in deep_iter_items(info_data, lambda x: is_loosely_str(x)):
    ...
    def _save_data(self, path, partial_save_keys=None):
    ...
    def save(self, save_path=None, auto_release=True, allow_overwrite=True, partial_save_keys=None, verbose=True):
        save_path = self._before_save(save_path, allow_overwrite, verbose)
        self._save_data(save_path, partial_save_keys=partial_save_keys)
    ...
    @classmethod
    def create_from_iter(cls, dir_path, iterator, length=None, **init_kwargs):
        obj = cls(**init_kwargs)
        obj.save(dir_path, verbose=False)
        ...
        for k, v in loop:
            cls._create_iter_step(obj, k, v)
        logger.success(f'{cls.__name__} create completely: {dir_path}')
        return obj
...
class LooseArray(LooseJSONTree):
    ...
    def insert(self, index, item, verbose=True):
    ...
    def append(self, item, verbose=True):
        self.insert(len(self), item, verbose=False)
        if verbose:
