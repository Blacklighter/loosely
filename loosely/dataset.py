...
from loosely.tree import LooseDict, LooseAtomDict, LooseArray, LooseBase
from loosely.utils import deep_iter_items
...
class LooseDataset(LooseDict):
    ...
    def _create_iter_step(cls, obj, k, v, info_autosave=True):
        obj.table.append(v, auto_release=False, info_autosave=info_autosave, verbose=False)

    @classmethod
    def _after_iter(cls, obj, dir_path, verbose):
        print('Clean excessive files in table...')
        obj.table._save_info()
        obj.table._clean_excessive_children()
        print('Clean completely.')
        if verbose:
            cls._log_create_success(dir_path)
        super()._after_iter(obj, dir_path, verbose=False)

    @classmethod
    def create_from_iter(cls, dir_path, iterator, length=None, meta={}, verbose=True):
        ...
        def it2():
            ...
            for item in iterator:
                ...
                yield i, item
                i += 1
        obj = super().create_from_iter(dir_path, it2(), length, verbose=False, meta=meta)
        return obj
